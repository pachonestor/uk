...
for c in [10**i for i in range(-4,9)]:
    for g in np.logspace(-10, 4, 15):
                        clf = SVC(kernel='rbf',random_state=0,C=c,gamma=g) 
                        startTime = time.time() 
                        ...
                        endTime = time.time()
                        results.append([c,g,scoretest,endTime - startTime,scoretrain])
                
results=pd.DataFrame(np.array(results))
results.columns=["C","gamma","test","Time","train"]
results=results.sort_values(by=["test"], ascending=False)
with pd.option_context('display.max_rows', None, 'display.max_columns', None):
    print(results)
results.to_csv("svmrbfsearch.csv")
