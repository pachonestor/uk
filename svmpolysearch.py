    ...
    for c in [10**i for i in range(-4,9)]:
        for g in np.logspace(-10, 4, 15):
            for b in [-16 + 4*i for i in range(9)]: 
                clf = SVC(kernel='poly',random_state=0,C=c,gamma=g, coef0=b,degree=d) 
                startTime = time.time() 
                ...
                endTime = time.time()
                results.append([scoretest,d,c,g,b,endTime - startTime,scoretrain])
                                       
results=pd.DataFrame(np.array(results))
results.columns=["test","degree","C","gamma","bias","Time","train"]
results=results.sort_values(by=["test"], ascending=False)
with pd.option_context('display.max_rows', None, 'display.max_columns', None):
    print(results)
results.to_csv("svmpolysearch.csv")
